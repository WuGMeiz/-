...
public class DelectOrgServlet extends HttpServlet {
    /**
	 * 
	 */
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(session.getAttribute("USER_ID")==null) {         
            ...
            int num = new MyTB_SEV_ORG_DAO().countczy(org_id,ts_id);
            if(num>0){
                out.print("errors-删除机构失败，请先删除操作员");
            }else{
                boolean b = new MyTB_SEV_ORG_DAO().delectOrg(org_id,org_name,ts_id);
                if(b==true){
                    String l_content="删除机构："+org_name;
                    new MyTBAdmin_Log().add_Log(l_content,user_id, tu_id);
                    out.print("sok-删除机构成功");
                    
                } 
                else
                {
                    out.print("errors-删除机构失败");
                }
            }
        }
    }
