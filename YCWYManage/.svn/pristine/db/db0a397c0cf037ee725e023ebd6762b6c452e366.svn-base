		...
		<div class="addBox">
			<table border="0" width="100%" cellpadding="0" cellspacing="0">
				<tr height="45">
					...
					<select id="Es_id" name="Es_id" class="input_txt" onchange="select_louyu(this,'louyuid','<%=request.getContextPath()+"/YCWYPage/BackPage/quarters/charge_select_ly.jsp"%>','<%=ts_id %>')" >
					...
					<td align="center">
                    	<input class="submit_input" name="chaxun" value=" 查 询 " onclick="select_TB_Build('show_update','<%=request.getContextPath()+"/YCWYPage/BackPage/quarters/lyadd_update.jsp"%>');return false;" type="button" />
                    </td>
                    <td align="center">
						 <% 
		                  if(new MyTB_SEV_ORG_DAO().getorg_level(session.getAttribute("ORG_ID").toString(),ts_id)<2)
		                   {
		                       %>	
		                   <input type="button" class="submit_input" onfocus="this.blur();" name="xinjjs" value="新建楼宇" style="margin-left: 10px" onclick="new_show_see_tanchu_div('div_app_show','','','<%=request.getContextPath()+"/YCWYPage/BackPage/quarters/ly_add_see.jsp"%>');return false;"/>	
		                <% 
		                    }
		                  else
		                     {
