...
<%@page import="WYBack_Stage.Dao.HostTypeDao"%>
...
<%
	...
	if(request.getParameter("type")==null) {
		...
		String Condition=" 1=1 and h.status='1' and v.status='1' and h.remark1='"+Es_id+"'";
		session.setAttribute("Condition",Condition);
		
		String Innerj = " h inner join TB_Estate_Village v on h.remark1=v.Es_id ";
	...
	Page pages=new Page();
	//表名
	pages.setTablename("TB_Estate_Housetype");
	...
	if(request.getParameter("pagenum")!=null) {
		...
		List<TB_Estate_Housetype> list = new HostTypeDao().getTB_HouseType(pages.getPagesize(),pages.getPagenum(),pages.getCondition());
...
<table border="0" width="100%" cellpadding="0" cellspacing="0" class="tableList"> 
			...
			<%
				...
				{
					TB_Estate_Housetype tev= (TB_Estate_Housetype) list.get(i);
			...
			%>
				...
				<tr height="38">
					...
					<td align="center">
					   <input type="text" class="input_txt1" id="ReName<%=i%>" name="ReName<%=i%>" value="<%=tev.getHtName() %>"  maxlength="15" />
					</td>
					<td align="center">
					  <%=tev.getEsName() %>
					</td>
				...
				<%-- 	<td align="center">
					...
					<input type="text" class="input_txt1" id="remark<%=i%>" name="remark<%=i%>" value="<%=tev.getRemark2() %>"  maxlength="50" />
					</td>
					
					<td align="center">
						<input width="45%" type="button" name="xiu<%=i%>" value=" 修改 "
							onclick="updatehostType('<%=tev.getHt_id() %>','<%=i%>','<%=request.getContextPath()%>/HostTypeServlet?arg=2','show_update','<%=request.getContextPath()+"/YCWYPage/BackPage/quarters/hostType_update.jsp?type=1"%>')" />
...
<%
		...
		String url = request.getContextPath()+"/YCWYPage/BackPage/quarters/hostType_update.jsp";
%>
	</tbody>
</table>
</div>
