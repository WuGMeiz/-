...
else 
{
	...
	if(request.getParameter("type")==null) {
		...
		String Condition=" 1=1 and a.status='1' and b.status='1'and c.status='1' and d.status='1' and e.status='1' and a.orderType='1' "+ts_id+Bu_id+Es_id+Un_id+EhNumber+create_time+payStatus+itemName+orderType+payType+OwnerName+LEVELS+sfy;
		session.setAttribute("Condition", Condition);
		//System.out.println(Condition);
		
		String Innerj = " a inner join TB_Estate_Village b on a.Es_id=b.Es_id inner join TB_Estate_Build c on a.Bu_id=c.Bu_id  inner join TB_Estate_House d on a.Eh_id=d.Eh_id inner join TB_Estate_item e on a.payItem=e.Ei_id left join TB_Estate_Unit f on d.Un_id=f.Un_id inner join TB_Estate_ChargeType g on a.payItem=g.Ei_id and g.Ht_id=d.EhType ";
...
<div style="width: 100%; overflow: hidden; overflow-x: auto;" >
  	<table align="center" class="tableList" width="120%" >
		...
		double znj_total_sum = 0.00;
	  	...
	  	<a onclick="new_show_see_tanchu_div2('select_orders_xq','','','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_xq.jsp"%>','<%=i%>');"><%=order.getEsName() %>--<%=order.getBuName() %><%if(!"".equals(order.getUnName())){%>--<%=order.getUnName() %><%} %>--<%=order.getEhNumber() %></a></td>
		...
		if(order.getPayStatus()==0){
			...
			%>
				<input type="hidden" id="total_znj<%=i %>" name="total_znj" value="<%=S_string.DecimalFormat_double(znj_total,2) %>"/>
		...
		<td align="center"><%if(order.getPayStatus()==0){%><font color='red'>未支付</font><%}else if(order.getPayStatus()==1){%><font color='green'>已支付</font><%}else if(order.getPayStatus()==2){%>部分支付<%}else if(order.getPayStatus()==3){%>已全额退款<%} %> </td>
	  	...
	  	<%if(order.getPayStatus()==1 || order.getTk_status().equals("1")){//已支付或者是全额退款 %>
	  	<td align="center"><button id="xjjf" name="xjjf" disabled="disabled" >线下支付</button></td>
	  	<%
	  	if(order.getTk_status().equals("1")){//全额退款
	  	%>
	  	...
	  	<%	
	  	  ...
	  	  if("2".equals(session.getAttribute("IsDrawBack").toString())){
  		%>
  		   <td align="center">
  		   ...
  		   <%}else{ %>
  		   <td align="center">
  		     <%-- <button id="xjtk" name="xjtk" onclick="update_orders_rgtk('<%=request.getContextPath()+"/TB_Estate_Order_uprgsfServlrt?args=2"%>','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_see.jsp?type=1"%>','<%=tsid%>','select_orders','<%=i%>');" >现金退款</button> --%>
  			<button id="xjtk" name="xjtk" onclick="update_orders_rgtk3('select_orders_xg','','','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_edit3.jsp?type=1"%>','<%=i%>','<%=tsid%>');" >线下退款</button>
  			</td>
  		   <%} %>
  		   <!-- SHL修改 start  -->
	  	...
	  	<td align="center"><button id="shanchu" name="shanchu" disabled="disabled" >删除</button></td>
	  	 <%}else{ %> 
	  	<td align="center">
	  		<%-- <button id="xjjf" name="xjjf" onclick="update_orders_rgsf('<%=request.getContextPath()+"/TB_Estate_Order_uprgsfServlrt?args=1"%>','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_see.jsp?type=1"%>','<%=tsid%>','select_orders','<%=i%>');" >现金支付</button> --%>
	  		<button id="xjjf" name="xjjf" onclick="update_orders_rgsf2('select_orders_xg','','800','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_edit2.jsp?type=1"%>','<%=i%>','<%=tsid%>');" >线下支付</button>
	  	</td>
	  	<td align="center"><button id="xjtk" name="xjtk" disabled="disabled" >线下退款</button></td>
	  	<td align="center"><button id="xjtk" name="xjtk" disabled="disabled" >普通凭证</button></td>
	  	<td align="center"><button id="xjtk" name="xjtk" disabled="disabled" >电子凭证</button></td>
	  	<td align="center"><button id="xiugai" name="xiugai" onclick="new_show_see_tanchu_div2('select_orders_xg','','','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_edit.jsp?type=1"%>','<%=i%>');">修改</button></td>
	  	<td align="center"><button id="shanchu" name="shanchu" onclick="delete_orders('<%=request.getContextPath()+"/TB_Estate_OrderServlrt"%>','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_see.jsp?type=1"%>','<%=tsid%>','select_orders','<%=i%>');">删除</button></td>
	  	<%} %>
	  	</tr>
	  	
	  	<%
	  
			znj_total_sum += znj_total;
		%>
	  	...
	  	<tr height="45">
			...
			<td style="text-align:left;" colspan="3">
				全选&emsp;<input type="button" value="批量删除订单" onclick="pldelTB_Estate_Order('orderid','<%=request.getContextPath()+"/TB_Estate_Order_plsc"%>','<%=request.getContextPath()%>/YCWYPage/BackPage/charge/charge_select_see.jsp?type=1','select_orders');return false;"/>
			<%-- <input type="button" value="批量现金支付" onclick="pldelTB_Estate_Order('orderid','<%=request.getContextPath()+"/TB_Estate_Order_plxj"%>','<%=request.getContextPath()%>/YCWYPage/BackPage/charge/charge_select_see.jsp?type=1','select_orders');return false;"/> --%>
			<input type="button" value="批量线下支付" 
			onclick="update_orders_plsf('select_orders_pl','','800','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/plrgsf.jsp?type=1"%>','orderid','<%=tsid%>');"/>
			</td>
			<td></td>
			<td></td>
			...
			<td align="center"><%=S_string.DecimalFormat_double(new MyTB_Estate_Order().getyjje(session.getAttribute("Condition").toString()),2)%></td>
			<td align="center"><%=S_string.DecimalFormat_double(new MyTB_Estate_Order().getZnjje(session.getAttribute("Condition").toString()),2)%></td>
			<td align="center"><%=S_string.DecimalFormat_double(new MyTB_Estate_Order().getsjje(session.getAttribute("Condition").toString()),2)%></td>
			<td colspan="8"></td>
	  	</tr>
	  	<%
	  	}else{ %>
	  	<tr height="45"><td  align="center" colspan="17">未查询到订单信息！！！</td></tr>
		<%} %>
		</tbody> 
  	</table>
  </div>
<%
String url = request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_see.jsp";
%>
<div class="pages">
