...
<% 
	...
	listi = new ChangeDao().getTBItem(yznr,ts_id,pageSize, pageNum,a_id,"物业费"); */
	
	List<TB_Estate_ChargeType> listi = new ArrayList<TB_Estate_ChargeType>();
	listi = new ChangeDao().getTBItemType(yznr,ts_id,pageSize, pageNum,a_id,"物业费");
...
<div class="addBoxList">
	<table border="0" width="100%" cellpadding="0" cellspacing="0" class="tableList">	
		<thead class="thead">
			<tr height="38">
				<th align="center" style="width: 6%">小区</th>
				<th align="center" style="width: 5%">房屋类型</th>
				<th align="center" style="width: 8%">收费方式</th>
				<th align="center" style="width: 3%">单价（元）</th>
				<th align="center" style="width: 3%">滞纳天数</th>
				<th align="center" style="width: 3%">滞纳金比例</th>
				<th align="center" style="width: 6%">备注</th>
		...
		<%
			...
			<tr height="38">
				...
				<td align="center">
					<input type="button" name="xiu<%=i%>" value=" 修改 " onclick="TBWuyeItem_update('1','<%=item.getCt_id() %>','<%=item.getEi_id() %>','<%=item.getCt_itemName() %>',
					'<%=i%>','<%=request.getContextPath()%>/TB_ItemUpdate?arg=1','show_update','<%=request.getContextPath()+"/YCWYPage/BackPage/calculate/WuYe_item_show.jsp?type=1"%>')" />
...
%>	
		<li onclick="javascript:Back('<%=request.getContextPath()%>/YCWYPage/BackPage/calculate/WuYe_item_show.jsp','<%=pageNum - 1%>','show_update')">上一页</li>
...
%>     
  	<li onclick="javascript:Next('<%=request.getContextPath()%>/YCWYPage/BackPage/calculate/WuYe_item_show.jsp','<%=pageNum + 1%>','show_update')">下一页</li>
  	
<%
  		}
  	%>
     <li> 到&nbsp;
	<select id="showpages" onchange="showpages('<%=request.getContextPath()%>/YCWYPage/BackPage/calculate/WuYe_item_show.jsp',this,'show_update')">
