...
/**
 * Servlet implementation class TB_Estate_ExoptionInsertServlet
 */
public class TB_Estate_ExoptionInsertServlet extends MutiFileUpload {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public TB_Estate_ExoptionInsertServlet() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	exoption.setTo_id(Integer.parseInt(parameters.get("To_id")));
		...
		exoption.setCreate_time(new T_time().getTime());
		exoption.setSort(1);
		exoption.setStatus(1);	
    	boolean bl = new TB_Estate_ExoptionDao().insert_Exoption(exoption);
    	if(bl){
    		/*HttpSession session = request.getSession();
    		session.setAttribute("To_id", request.getParameter("To_id").toString());*/
    		//out.print("<script>alert('修改成功');");
    		out.print("<script>alert('添加成功');parent.showselect('"+return_divid+"','"+return_url+"');parent.close_tanchu_div('"+close_divid+"');</script></script>");
    	}else{
    		//out.print("<script>alert('修改失败');");
    		out.print("<script>alert('添加失败');parent.showselect('"+return_divid+"','"+return_url+"');parent.close_tanchu_div('"+close_divid+"');</script></script>");
    	}
