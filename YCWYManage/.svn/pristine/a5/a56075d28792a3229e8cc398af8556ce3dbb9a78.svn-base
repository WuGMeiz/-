...
<% 
	...
	listi = new ChangeDao().getTBItem(yznr,ts_id,pageSize, pageNum,a_id,"电费"); */
	
	List<TB_Estate_ChargeType> listi = new ArrayList<TB_Estate_ChargeType>();
	listi = new ChangeDao().getTBItemType(yznr,ts_id,pageSize, pageNum,a_id,"电费");
	int countNum = 0;
	int pageCount = 0;
	if (listi.size() > 0) {
	...
	<table border="0" width="130%" cellpadding="0" cellspacing="0" class="tableList">	
		...
		<%
			...
			<tr height="38">
				...
				<td align="center">
					<input type="text"  class="input_txt" style="width: 90%;" id="price<%=i%>" name="price<%=i%>" value="<%=S_string.DecimalFormat_string(item.getCt_price(), 3) %>" />
				</td>	
				<td align="center">
			...
			<%-- 	<td align="center">
				...
				<td align="center">
					<input type="button" name="xiu<%=i%>" value=" 修改 " onclick="TBEleItem_update('1','<%=item.getCt_id() %>','<%=item.getEi_id() %>','<%=item.getCt_itemName()%>',
					'<%=i%>','<%=request.getContextPath()%>/TB_ItemUpdate?arg=6','show_update','<%=request.getContextPath()+"/YCWYPage/BackPage/calculate/Electric_item_show.jsp?type=1"%>')" />
