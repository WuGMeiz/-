...
<% 
	...
	listi = new ChangeDao().getTBItem(yznr,ts_id,pageSize, pageNum,a_id,"供暖费"); */
	List<TB_Estate_ChargeType> listi = new ArrayList<TB_Estate_ChargeType>();
	listi = new ChangeDao().getTBItemType(yznr,ts_id,pageSize, pageNum,a_id,"供暖费");
		...
		<%
			...
			<tr height="38">
				...
				<td align="center">
					 ...
					 <input type="hidden"  class="input_txt" style="width: 90%;" id="guRatio<%=i%>" name="guRatio<%=i%>" value="<%=item.getCt_guRatio() %>" />
				</td>
				<td align="center">
					 <input type="text"  class="input_txt" style="width: 90%;" id="remark<%=i%>" name="remark<%=i%>" value="<%=item.getCt_remark()%>" />
				</td>
				<td align="center">
					<input type="button" name="xiu<%=i%>" value=" 修改 " onclick="TBGongReItem_update('1','<%=item.getCt_id() %>','<%=item.getEi_id() %>','<%=item.getCt_itemName() %>',
					'<%=i%>','<%=request.getContextPath()%>/TB_ItemUpdate?arg=2','show_update','<%=request.getContextPath()+"/YCWYPage/BackPage/calculate/GongRe_item_show.jsp?type=1"%>')" />
...
%>	
		<li onclick="javascript:Back('<%=request.getContextPath()%>/YCWYPage/BackPage/calculate/GongRe_item_show.jsp','<%=pageNum - 1%>','show_update')">上一页</li>
...
%>     
  	<li onclick="javascript:Next('<%=request.getContextPath()%>/YCWYPage/BackPage/calculate/GongRe_item_show.jsp','<%=pageNum + 1%>','show_update')">下一页</li>
  	
<%
  		}
  	%>
     <li> 到&nbsp;
	<select id="showpages" onchange="showpages('<%=request.getContextPath()%>/YCWYPage/BackPage/calculate/GongRe_item_show.jsp',this,'show_update')">
