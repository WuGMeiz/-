	...
	<table width="100%" class="tableTbody" >
		...
		<tr height="45">
	 	...
	 	<tr height="45">
			...
			<td><% if(order.getPayType().equals("")){%><%}else if(order.getPayType().equals("0")){%>网上支付<%}else if(order.getPayType().equals("1")){%>现金支付<%}else if(order.getPayType().equals("2")){%>扫码盒子<%}else if(order.getPayType().equals("3")){%>转账支付<%}else if(order.getPayType().equals("4")){%>刷卡支付<%}else if(order.getPayType().equals("5")){%>调账支付<%}%>
		   </td>
		</tr>
		<tr height="45">
		...
		<tr height="45">
			<td  align="center" colspan="8">
				<input type="button"  class="submit_input" value="确认退款" onclick="update_orders_rgtk2('<%=request.getContextPath()+"/TB_Estate_Order_uprgsfServlrt?args=2"%>','<%=request.getContextPath()+"/YCWYPage/BackPage/charge/charge_select_see.jsp?type=1"%>','select_orders');"> 			
			</td>
		</tr>
	</table>
</div>
