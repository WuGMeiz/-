...
else 
{
	...
	if(request.getParameter("type")==null) {
		...
		String Condition=" 1=1 and a.status='1' and b.status='1'and c.status='1' and d.status='1'  and a.orderType='3' "+ts_id+Bu_id+Un_id+Es_id+EhNumber+create_time+payStatus+orderType+payType+OwnerName+itemName+LEVELS;
  	...
  	<table align="center" class="tableList" width="110%" >
		...
		<% if(list.size()>0){
	  	...
	  	<td align="center" colspan="3">
		  	...
		  	<%=order.getEsName() %>--<%=order.getBuName() %><%if(!"".equals(order.getUnName())){%>--<%=order.getUnName() %><%} %>--<%=order.getEhNumber() %>
	  	</td>
		...
		<td align="center"><%if(order.getPayStatus()==0){%><font color='red'>未支付</font><%}else if(order.getPayStatus()==1){%><font color='green'>已支付</font><%}else if(order.getPayStatus()==2){%>部分支付<%} else if(order.getPayStatus()==3){%>已全额退款<%} %></td>
	  	...
	  	<tr><td height="45" align="center" colspan="11">未查询到订单信息！！！</td></tr>
		<%} %>
		</tbody>
  	</table>
 </div>
<%
String url = request.getContextPath()+"/YCWYPage/BackPage/baobiao/ge_baobiao_see.jsp";
%>
<div class="pages">
